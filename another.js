...
function renderProducts(){
  for(var i=0;i<products.length;i++){
    ...
    div.innerHTML = '<img src="'+products[i].img+'" alt="'+products[i].name+'"><h3>'+products[i].name+'</h3><p>रु'+products[i].price+'</p><button onclick="addToCart('+products[i].id+')">Add to Cart</button>';
...
function renderCart(){
  ...
  for(var i=0;i<cart.length;i++){
    ...
    li.innerText = cart[i].name+" - रु"+cart[i].price;
